    ...
    public static class Hooks
    {
        ...
        private static void Attack_DoMeleeAttack(On.Attack.orig_DoMeleeAttack orig, Attack self)
        {
            ...
            if (IsLocalPlayer(self.m_character))
            {
                ...
                //Jotunn.Logger.LogDebug($"Returning states to false.");
            }
        }
        ...
        private static void Character_RPC_Damage(On.Character.orig_RPC_Damage orig, Character self, long sender, HitData hit)
        {
            ...
            if (attacker)
            {
                //Jotunn.Logger.LogDebug($"Primary: {States.PrimaryAttack}, Secondary: {States.SecondaryAttack} with {hit.m_skill}. Attackers {(attacker as Player).GetCurrentWeapon()} is a {(attacker as Player).GetCurrentWeapon().m_shared.m_skillType}. alerted?: {alerted}");
                switch (hit.m_skill)
                {
                    case Skills.SkillType.Knives:
                        //Jotunn.Logger.LogDebug($"{self.m_baseAI},{!alerted}{hit.m_backstabBonus > 1f}{time - backStabTime > 300f}, Evaluates to: {(self.m_baseAI != null && !alerted && hit.m_backstabBonus > 1f && Time.time - self.m_backstabTime > 300f)}");
                    ...
                    case Skills.SkillType.Clubs:
                        ...
                        {
                            XP = DamageToXP(self, hit);
                            NotificationHandler.Instance.AddNotification($"{hit.m_skill}: <color={highlightColour}>+{XP:0.00}</color>XP");
                            attacker.RaiseSkill(hit.m_skill, XP);
                        }
                        break;
                ...
                Jotunn.Logger.LogDebug($"{hit.GetAttacker().GetHoverName()} performed {hit.m_skill} against {self.GetHoverName()} hitting for {hit.GetTotalDamage()} yielding {XP}.");
            }
        }
                    ...
                    if (c != null && c.IsMonsterFaction())
                    {
                        ...
                        switch (hitData.m_skill)
                        {
                            ...
                            case Skills.SkillType.Spears:
                                ...
                                Jotunn.Logger.LogDebug($"{(self.m_owner as Player).GetPlayerName()} hit {c.m_name} at {distance:0.0}m for {hitData.GetTotalDamage():0.00}/{c.GetMaxHealth()} using a {multi:0.00} multiplier yeilding {XP:0.00}XP to {hitData.m_skill}.");
        ...
        private static void Humanoid_BlockAttack(ILContext il)
        {
            ...
            {
                num /= 10; //0.5-10
                Jotunn.Logger.LogDebug($"Gave {chara.m_name} {num} XP for Parry: {num}");
        ...
        {
            Jotunn.Logger.LogDebug($"override: {damageOverride} vs dmg: {hit.GetTotalDamage()}");
            ...
            if (num < 1) num = 1;
            Jotunn.Logger.LogDebug($"override: {damageOverride} vs dmg: {hit.GetTotalDamage()}, MaxHP: {victim.GetMaxHealth()}, post-Clamp: {dmg}");
            return num;
        }
